...
/**
 * Draw a ayat on a random background image.
 * @param {{category: string,  ayat: string, description: string}} ayat - an object with the following properties:
 *   - category {string} - category of the ayat
 *   - ayat {string} - the ayat itself
 *   - description {string} - description of the ayat
 ...
 *   - description {string} - description of the ayat
 */
module.exports = async function drawayat(ayat) {


  const randomText = ayat;
  const wordCount = randomText.ayat.split(' ').length;
  ...
  const y = canvas.height / 1.75 - fontSize / 2;
  ...
  wrapText(ctx, randomText.ayat, x, y, canvas.width - 170, 80);
