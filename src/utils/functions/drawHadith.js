...
/**
 * Draw a arab on a random background image.
 * @param {{category: string, number: string, arab: string, description: string}} arab - an object with the following properties:
 *   - category {string} - category of the arab
 *   - number {string} - number to the arab
 *   - arab {string} - the arab itself
 *   - description {string} - description of the arab
 ...
 *   - description {string} - description of the arab
 */
module.exports = async function drawarab(arab) {


  const randomText = arab;
  const wordnumber = randomText.arab.split(' ').length;
  ...
  ctx.fillText(randomText.number, canvas.width / 2, 30);

  ctx.fillStyle = 'white';
  ctx.font = 'italic 35px Amiri';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'bottom';
  ctx.fillText(randomText.category, 20, canvas.height - 20);
  ...
  const fontSize = Math.max(25, Math.min(60, 60 - (wordnumber - 20) * 2));
  ...
  wrapText(ctx, randomText.arab, x, y, canvas.width - 170, 80);
